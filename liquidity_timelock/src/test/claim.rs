...
fn claim_test_neet_to_wait() {
    ...
    test.timelock_contract
        .initialize(&test.admin, &test.router_contract.address, &end_timestamp);
    ...
    let result = test.timelock_contract.try_claim(&pair_address);

    assert_eq!(
        result,
        Err(Ok(CombinedLiquidityTimelockError::TimelockNeedToWait))
    );
}
...
fn claim_test() {
    ...
    // NOW WE WILL CLAIM
    test.timelock_contract.claim(&pair_address);
    ...
    assert_eq!(
        pair_client.balance(&test.admin),
        initial_admin_lp_token_balance + expected_liquidity
    );
    ...
    assert_eq!(
        ...
        initial_admin_balance_0.checked_add(amount_0).unwrap()
    );
    assert_eq!(
        test.token_1.balance(&test.admin),
        initial_admin_balance_1.checked_add(amount_1).unwrap()
    );
}
