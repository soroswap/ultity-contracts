...
export async function liquidityTimelock(addressBook: AddressBook, contractKey: string) {
  ...
  console.log('-------------------------------------------------------');
  let xtarUserBalance = await invokeCustomContract("CAPCD5BA3VYK4YWTXUBBXKXXIETXU2GGZZIQ4KDFI4WWTVZHV6OBIUNO", "balance", [new Address(loadedConfig.admin.publicKey()).toScVal()], loadedConfig.admin, true)
  console.log('XTAR USER BALANCE:', scValToNative(xtarUserBalance.result.retval));
  let xtarContractBalance = await invokeCustomContract("CAPCD5BA3VYK4YWTXUBBXKXXIETXU2GGZZIQ4KDFI4WWTVZHV6OBIUNO", "balance", [new Address(addressBook.getContractId(contractKey)).toScVal()], loadedConfig.admin, true)
  console.log('XTAR CONTRACT BALANCE:', scValToNative(xtarContractBalance.result.retval));
  let usdcUserBalance = await invokeCustomContract("CCKW6SMINDG6TUWJROIZ535EW2ZUJQEDGSKNIK3FBK26PAMBZDVK2BZA", "balance", [new Address(loadedConfig.admin.publicKey()).toScVal()], loadedConfig.admin, true)
  console.log('USDC USER BALANCE:', scValToNative(usdcUserBalance.result.retval));
  let usdcContractBalance = await invokeCustomContract("CCKW6SMINDG6TUWJROIZ535EW2ZUJQEDGSKNIK3FBK26PAMBZDVK2BZA", "balance", [new Address(addressBook.getContractId(contractKey)).toScVal()], loadedConfig.admin, true)
  console.log('USDC CONTRACT BALANCE:', scValToNative(usdcContractBalance.result.retval));
  let lpUserBalance = await invokeCustomContract("CAEHRFL6HROIYFQK62ATJVRYEMLATWXVUYOBWCR5T7MFJB23BK56QD5J", "balance", [new Address(loadedConfig.admin.publicKey()).toScVal()], loadedConfig.admin, true)
  console.log('LP USER BALANCE:', scValToNative(lpUserBalance.result.retval));
  let lpContractBalance = await invokeCustomContract("CAEHRFL6HROIYFQK62ATJVRYEMLATWXVUYOBWCR5T7MFJB23BK56QD5J", "balance", [new Address(addressBook.getContractId(contractKey)).toScVal()], loadedConfig.admin, true)
